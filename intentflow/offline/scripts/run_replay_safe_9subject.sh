#!/usr/bin/env bash
# =============================================================
# Run tcformer_replay_safe_otta on all 9 BCIC2a subjects, reusing
# the source checkpoints from the existing a1_9subject sweep.
# Aggregates results alongside the existing 4 variants.
#
# Usage:
#   ./scripts/run_replay_safe_9subject.sh [GPU_ID]
# =============================================================
...
REPLAY_CONFIG="configs/tcformer_replay_safe_otta/tcformer_replay_safe_otta_s2_smoke.yaml"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BASE_DIR="results/replay_safe_9subject_${TIMESTAMP}"
...
echo "================================================="
echo "Replay-safe OTTA 9-subject sweep"
echo "Output: ${BASE_DIR}"
echo "================================================="

for sid in 1 2 3 4 5 6 7 8 9; do
    ckpt=$(ckpt_for_subject "${sid}")
    out_dir="${BASE_DIR}/eval/s${sid}/replay_safe_default"
    log_path="${BASE_DIR}/eval/s${sid}_replay_safe_default.log"
...
# Combine with existing a1 sweep results into a 5-variant summary.
python - "${BASE_DIR}" "${A1_DIR}" <<'PY'
...
variants_a1 = ["source_only", "proto_otta_default", "policy_safe_default", "policy_safe_no_shallow"]
table = {v: {} for v in variants_a1}
for v in variants_a1:
    for s in subjects:
        table[v][s] = parse_acc(a1 / "eval" / f"s{s}" / v)

table["replay_safe_default"] = {}
for s in subjects:
    table["replay_safe_default"][s] = parse_acc(base / "eval" / f"s{s}" / "replay_safe_default")

variants = variants_a1 + ["replay_safe_default"]
...
md = ["# Replay-safe OTTA 9-subject sweep (with a1 baselines)", ""]
