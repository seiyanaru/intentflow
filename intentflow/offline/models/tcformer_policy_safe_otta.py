"""TCFormer wrapper for Policy-SafeCommit OTTA."""
...
from models.policy_safe_commit_otta import PolicySafeCommitOTTA
...
class TCFormerPolicySafeOTTA(ClassificationModule):
    """TCFormer with StateExtractor -> Policy -> SafeCommit control layer."""
    ...
    def __init__(self, n_classes: int, **kwargs):
        ...
        self.policy_otta = None
        self.train_dataloader_ref = None
        self.test_policy_records: List[Dict[str, Any]] = []
    ...
    def on_test_start(self):
        ...
        print("[TCFormerPolicySafeOTTA] Initializing Policy-SafeCommit OTTA...")
        self.policy_otta = PolicySafeCommitOTTA(
            model=self.model,
            n_classes=self.n_classes,
            **self.policy_safe_kwargs,
        )
        self.policy_otta.to(self.device)

        try:
            datamodule = getattr(self.trainer, "datamodule", None)
            ch_names = self._find_channel_names(datamodule)
            if ch_names:
                self.policy_otta.set_channel_roles(get_electrode_roles(ch_names))
            else:
                print("[TCFormerPolicySafeOTTA] Warning: channel names unavailable; neuro state disabled.")
        except Exception as exc:
            print(f"[TCFormerPolicySafeOTTA] Warning: failed to set channel roles: {exc}")

        if self.train_dataloader_ref is not None:
            self.policy_otta.compute_source_statistics(self.train_dataloader_ref, device=self.device)
        else:
            print("[TCFormerPolicySafeOTTA] Warning: no train dataloader; source criteria unavailable.")
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.policy_otta is not None and self.enable_otta:
            result = self.policy_otta(x, return_debug=True)
            ...
            self.test_policy_records.append(record)
        else:
    ...
    def on_test_epoch_end(self):
        if self.test_policy_records and self.subject_id != "unknown":
            os.makedirs(self.results_dir, exist_ok=True)
            stats_path = os.path.join(
                self.results_dir,
                f"policy_safe_otta_stats_s{self.subject_id}_{self.model_name}.npz",
            )
            keys = sorted(self.test_policy_records[0].keys())
            save_dict = {}
            for key in keys:
                values = [row.get(key) for row in self.test_policy_records]
                if isinstance(values[0], str):
                    save_dict[key] = np.asarray(values, dtype="U128")
                else:
                    save_dict[key] = np.asarray(values, dtype=np.float32)
            np.savez(stats_path, **save_dict)
            print(f"[TCFormerPolicySafeOTTA] Saved Policy-SafeCommit stats to {stats_path}")

            if self.policy_otta is not None:
                self.policy_otta.print_stats()
            self.test_policy_records = []
